...
# solution for mentioned error: setting host to "dec2021.archive.ensembl.org"
...
# merge original dataframe with Biomart datasets
# mouse_balzer <- merge(mouse_balzer, genesV2_mouse_balzer, by="gene", sort = F)

# omit any NA values 
# mouse_balzer <-na.omit(mouse_balzer) #no NA values existent

# remove duplicate genes
# ids = ids[!duplicated(ids[c("gene")]),]
...
mouse_balzer <- merge(mouse_balzer, genesV2_mouse_balzer, by="gene", sort = F) # 4012   10
...
write.csv(mouse_kirita, "Kirita__mouse__human_genenames.csv", row.names = F)
